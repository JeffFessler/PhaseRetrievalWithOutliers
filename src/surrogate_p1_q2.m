function f = surrogate_p1_q2(x,s,Ax,y,weights,beta,check_use_y_s)
% f = surrogate_p1_q2(x,s,Ax,y,weights,beta,check_use_y_s)
%
% Inputs:
%  x - signal value
...
%
% This function computes the value of the majorizer function for 1-norm
...
f = sum(col(weights.*(yleq0.*hplus(t,y)+(~yleq0).*phi(t,s,y)))) + beta*sum(abs(col(x)));
