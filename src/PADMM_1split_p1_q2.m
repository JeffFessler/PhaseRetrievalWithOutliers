function [x_best,Phi_best,Ax_best,u_best,lm_best,nAmults,mus_last,info] = PADMM_1split_p1_q2(x0,s,A,y,weights,beta,varargin)
% [x_best,Phi_best,Ax_best,u_best,lm_best,nAmults,mus_last,info] = PADMM_1split_p1_q2(x0,s,A,y,weights,beta,key1,value1,key2,value2,...)
%
% Inputs:
%  x0 - initial guess
...
% proposed method with the 1-norm data fit term. It iterates the x-update
% (using either shrinkage if A is unitary or FISTA-like shrinkage if A is
% not), the u-update (using the calculations in the paper), and the
% lagrange multipliers update. The algorithm also includes (optional)
% automatic tuning of mu.
%
...
opts.Ax0 = []; % allow precomputed A*x0
...
Phifun = @(x,Ax) surrogate_p1_q2(x,s,Ax,y,weights./opts.norm_factor,beta);
f1fun = @(u,d,eta) abs(u).^2-y + eta.*abs(u-d).^2;
f2fun = @(u,d,eta) y + abs(s).^2 - 2*real(conj(s).*u) + eta.*abs(u-d).^2;
...
for imu = 1:nmus
    ...
    while iter < opts.I_ADMM
        ...
        % min f_1
        u1 = (etas./(1+etas)).*d;
        use1 = f1fun(u1,d,etas) >= f2fun(u1,d,etas);

        % min f_2
        u2 = (1./etas).*s + d;
        use2 = f2fun(u2,d,etas) >= f1fun(u2,d,etas);

        % min (f_1=f_2=T)
        c1 = sqrt(2*(y+abs(s).^2));
        u3 = c1.*exp(complex(0,angle((1+etas).*s+etas.*d)))-s;
        use3 = ~(use1 | use2);
