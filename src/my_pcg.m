  function [xs, info] = my_pcg(x, A, b, varargin)
%|function [xs, info] = my_pcg(x, A, b, [options])
%|
%| preconditioned conjugate gradients (PCG) algorithm
%| for solving Ax = b, where A is (Hermitian) spd
%|
%| in
%|	x	[n 1]		initial estimate
%|	A	[n n]		Gram matrix
%|	b	[n 1]		data
%|
%| options
...
%|	precon	[n n]		preconditioner (matrix or object) (or 1)
...
%|
%| Copyright July 2014, Daniel Weller and Jeff Fessler, University of Michigan
...
arg.precon = 1;
arg.niter = 1;
arg.isave = 'last'; % default according to iter_saver()
...
if arg.stop_grad_tol
	norm_grad = @(g) norm(g, arg.stop_grad_norm) / norm(b, arg.stop_grad_norm); % DW - changed
end
...
for iter = 1:arg.niter
	...
	newinprod = reale(newinprod, 'warn');
	if iter == 1
		ddir = pregrad;
		gamma = 0;
	else
...
%			gamma = (newinprod - oldgrad' * pregrad) / oldinprod;
	...
	if arg.dircheck && real(ddir' * ngrad) < 0
		...
		ratio = norm(ngrad(:), arg.stop_grad_norm) / norm(b, arg.stop_grad_norm); % DW - changed
    ...
    denom = abs(ddir'*Adir); % DW - changed
	if denom == 0
