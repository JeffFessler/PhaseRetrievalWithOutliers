function [x_best,f_best,info] = phase_retrieval_p2_q2(sz_x,A,y,beta,varargin)
% [x_best,f_best,info] = phase_retrieval_p2_q2(sz_x,A,y,beta,key1,value1,key2,value2,...)
%
% Inputs:
...
% bulk of the proposed algorithm is in ox_fessler_p2_q2.m and in
% PADMM_1split_p2_q2.m, so a detailed description of the algorithm can be 
% found in those places. This code implements the quadratic data fit term
% for squared-magnitude (q = 2) measurements.
%
...
while (ii < opts.Ninit_min && nAmults < opts.I_mults_max) || (nAmults < opts.I_mults_min && ii < opts.Ninit_max)
    ...
    if exist('info','var')
        [x,f,nAmults,ox_info] = ox_fessler_p2_q2(x,s,A,y,beta,opts.ox_opts{:},'Ax0',Ax,'norm_factor',opts.norm_factor);
        info.fs(ii) = f;
    else
        [x,f,nAmults] = ox_fessler_p2_q2(x,s,A,y,beta,opts.ox_opts{:},'Ax0',Ax,'norm_factor',opts.norm_factor);
    end
