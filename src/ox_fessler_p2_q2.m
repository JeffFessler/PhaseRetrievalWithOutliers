function [x,f,nAmults,info] = ox_fessler_p2_q2(x,s,A,y,beta,varargin)
% [x,f,nAmults,info] = ox_fessler_p2_q2(x,s,A,y,beta,key1,value1,key2,value2,...)
%
% Inputs:
...
% PADMM_1split_p2_q2.m to find a new x that minimizes the majorizer, and
...
opts.ADMM_opts = {}; % options for ADMM/PADMM (see PADMM_1split_p2_q2.m)
opts.Ax0 = []; % (optional) precomputed A*x
opts.norm_factor = 1;

opts = vararg_pair(opts,varargin);

Psifun = @(x,Ax) norm(col(y-abs(Ax).^2),2)^2./opts.norm_factor + beta*sum(abs(col(x))); % true objective
...
while iter < opts.I_ox
    ...
    Phi_prev = surrogate_p2_q2(x,s,Ax,y,weights./opts.norm_factor,beta);
    ...
    % update x using ADMM
    if exist('info','var')
        [x,Phi,Ax,u,lm,nAmults,mus_last,info.ADMM_info{iter}] = PADMM_1split_p2_q2(x,s,A,y,weights,beta,opts.ADMM_opts{:},opts_extra{:});
    else
        [x,Phi,Ax,u,lm,nAmults,mus_last] = PADMM_1split_p2_q2(x,s,A,y,weights,beta,opts.ADMM_opts{:},opts_extra{:});
    end
